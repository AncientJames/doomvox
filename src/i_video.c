...
[[maybe_unused]] static const char rcsid[] = "$Id: i_x.c,v 1.6 1997/02/03 22:45:10 b1 Exp $";
...
void cmap_to_fb(uint8_t * out, uint8_t * in, int in_pixels)
{
    ...
    {
        c = current_palette[*in];  /* R:8 G:8 B:8 format! */
...
void I_SetPalette (byte* palette)
{
    ...
    for (i=0; i<256; ++i ) {
        ...
        current_palette[i].r = gammatable[usegamma][*palette++];
        current_palette[i].g = gammatable[usegamma][*palette++];
        current_palette[i].b = gammatable[usegamma][*palette++];
    }
