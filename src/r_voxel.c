...
int VX_PaletteIndex (byte * pal, int r, int g, int b)
{
	int best = 0;
	int best_dist = (1 << 30);
...
void VX_CreateRemapTable (byte * p, byte * table)
{
...
void VX_RemapSlabColors (struct Voxel * v, int x, int y, byte * table)
{
...
struct Voxel * VX_Decode (byte * p, int length)
{
	// too short?
	if (length < 40 + 768)
		return NULL;
...
bool VX_Load (int spr, int frame)
{
...
static struct VisVoxel visvoxels[MAXVISSPRITES];
...
int VX_RotateModeForThing (mobj_t * thing)
{
...
bool VX_CheckFrustum (fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2,
...
{
	// WISH: support low detail mode
	if (detailshift)
		return false;

	// when this happens, we have also run out of vissprites
	if (num_visvoxels >= MAXVISSPRITES)
		return false;
	...
	// create the VisVoxel...
	struct VisVoxel * vv = &visvoxels[num_visvoxels];
	num_visvoxels += 1;
...
void VX_DrawColumn (vissprite_t * spr, int x, int y)
{
	...
	bool shadow = ((spr->mobjflags & MF_SHADOW) != 0);
	...
	for (; ux < ((Cx > Bx) ? Cx : Bx) ; ux += FRACUNIT)
	{
		...
		for (; slab < end ; slab += len)
		{
			...
			bool has_side = ((face & (ux > Bx ? B_face : A_face)) != 0);
			...
			bool has_top    = ((face & F_TOP) && top_z < 0);
			bool has_bottom = ((face & F_BOTTOM) && top_z > ((int)len << FRACBITS));
			...
			if (has_top)
			{
				...
				byte pix = spr->colormap[slab[0]];

				for (; uy < uy1 ; uy += FRACUNIT)
				{
					dest[(uy >> FRACBITS) * SCREENWIDTH + (ux >> FRACBITS)] = pix;
				}
			}
			else if (has_bottom)
			{
				...
				byte pix = spr->colormap[slab[len - 1]];
...
bool VX_HasModel(mobj_t* thing) {
	return all_voxels[thing->sprite][thing->frame & FF_FRAMEMASK] != NULL;
}

void VX_WorldBounds(mobj_t* thing, vertex_t corners[4]) {
	struct Voxel * v = all_voxels[thing->sprite][thing->frame & FF_FRAMEMASK];
	if (v) {
		corners[0].x = corners[3].x = thing->x /*- v->x_pivot*/ - (v->x_size << (FRACBITS-1));
		corners[1].x = corners[2].x = thing->x /*- v->x_pivot*/ + (v->x_size << (FRACBITS-1));
		corners[0].y = corners[1].y = thing->y /*- v->y_pivot*/ - (v->y_size << (FRACBITS-1));
		corners[2].y = corners[3].y = thing->y /*- v->y_pivot*/ + (v->y_size << (FRACBITS-1));
	}
}
...
uint8_t VX_SampleModel(mobj_t* thing, fixed_t wx, fixed_t wy, fixed_t wz) {
	...
	int x = (wx - thing->x + v->x_pivot + (FRACUNIT/2)) >> FRACBITS;
	if (x < 0 || x >= v->x_size) {
		return 0;
	}

	int y = (wy - thing->y + v->y_pivot + (FRACUNIT/2)) >> FRACBITS;
	if (y < 0 || y >= v->y_size) {
		return 0;
	}

	int z = (-(wz - thing->z) + v->z_pivot) >> FRACBITS;
	if (z < 0 || z >= v->z_size) {
		return 0;
	}
...
void VX_Blit(mobj_t * thing, fixed_t scaleh, fixed_t scalev, angle_t rotation) {
	...
	fixed_t ox = thing->x + (FRACUNIT / 2) - viewx;
	fixed_t oy = thing->y + (FRACUNIT / 2) - viewy;
	fixed_t oz = thing->z + (FRACUNIT / 2) - viewplayer->mo->z;

	fixed_t rx = finecosine[rotation >> ANGLETOFINESHIFT];
	fixed_t ry =   finesine[rotation >> ANGLETOFINESHIFT];
	fixed_t orx = FixedMul(ox, ry) + FixedMul(oy,-rx);
	fixed_t ory = FixedMul(ox, rx) + FixedMul(oy, ry);

	fixed_t dx = finecosine[(rotation-thing->angle) >> ANGLETOFINESHIFT];
	fixed_t dy =   finesine[(rotation-thing->angle) >> ANGLETOFINESHIFT];

	//dc_colormap = spritelights[0];

	for (int y = 0; y < v->y_size; ++y) {
		for (int x = 0; x < v->x_size; ++x) {

			fixed_t sx = (x << FRACBITS) - v->x_pivot;
			fixed_t sy = (y << FRACBITS) - v->y_pivot;
			fixed_t sdx = FixedMul(sx, dx) + FixedMul(sy, dy);
			fixed_t sdy = FixedMul(sx,-dy) + FixedMul(sy, dx);

			int vx = (FixedMul(sdx + orx, scaleh) >> FRACBITS) + (VOXELS_X / 2);
			int vy = (FixedMul(sdy + ory, scaleh) >> FRACBITS) + (VOXELS_Y / 2);

			if (vx >= 0 && vy >= 0 && vx < VOXELS_X && vy < VOXELS_Y) {
				...
				while (slab < end) {
					...
					if (face) {
						for (int i = 0 ; i < len ; ++i) {
							int vz = (FixedMul(((-i - top) << FRACBITS) + v->z_pivot + oz, scalev) >> FRACBITS) + (VOXEL_GROUND);
							if (vz >= 0 && vz < VOXELS_Z) {
								VX_Plot(vx, vy, vz, slab[i]);
							}
						}
					}
					slab += len;
